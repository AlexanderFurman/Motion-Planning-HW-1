...
def get_minkowsky_sum(original_shape: Polygon, r: float) -> Polygon:
    ...
    while cond==0:
        ...
        if v_dx==0:
            ...
            angle_v=(v_dy)/(v_dx)
        w_dx=(v.boundary.coords.xy[0][(j+1)%m]-v.boundary.coords.xy[0][j%m])
        w_dy=v.boundary.coords.xy[1][(j+1)%m]-v.boundary.coords.xy[1][j%m]
        if w_dx==0:
            angle_w=999
        else:
            angle_w = (w_dy)/(w_dx)
